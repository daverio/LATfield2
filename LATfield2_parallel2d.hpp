...
void Parallel2d::initialize(int proc_size0, int proc_size1,int IO_total_size, int IO_node_size)
{
	...
	//initialize MPI and set the world comm...
	...
	this->initialize(proc_size0, proc_size1, IO_total_size, IO_node_size, world_comm);

}

void Parallel2d::initialize(int proc_size0, int proc_size1, MPI_Comm & world_comm){
	this->initialize(proc_size0, proc_size1, 0 , 0 , world_comm);
}

void Parallel2d::initialize(int proc_size0, int proc_size1,int IO_total_size, int IO_node_size, MPI_Comm & world_comm){


	#ifndef EXTERNAL_IO
		lat_world_comm_ = world_comm;
...
Parallel2d::~Parallel2d()
{
  ...
  if((!finalized) && (!neverFinalizeMPI) && mpi_initialized_ ) { MPI_Finalize(); }
}
