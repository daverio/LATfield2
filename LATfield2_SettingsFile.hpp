...
/*! \class SettingsFile  
 \brief A class desinged to make reading in runtime settings easier.
 \author M. Hindmarsh
 ...
 MPI. To use this class in serial code the preprocessor defintion SERIAL must be set. 
...
public:
  ...
  static int autoCreate;
    ...
    /*!
     ...
     \param parameter: pointer to the variable where the parameter will be assigned. If the parameter does not exite and the mode autocreate is set, this method will add the parameter to the settings file with the current value of "parameter". In the case the mode is set to nocreate, then read will exit for security, for this reason in it is always advise to set the mode to nocreate for production runs.
     */
