...
#include <string.h>
   ...
   {	      
	   ...
	   dset_id = H5Dcreate1(file_id, "field", dtype_id, filespace,plist_id);
...
//	   delete[] filename;	   
	   ...
	   for(p=0;p < mpi_size;p++)
	   {
		   ...
		   if(mpi_rank==p)
		   {
			   ...
			   dset_id = H5Dopen(file_id, "/field", H5P_DEFAULT);
			   filespace = H5Dget_space(dset_id);
...
//	   free(filename);
	...
	int load_hdf5_externC(char *data,long file_offset[2],int *size,int * sizeLocal,int halo, int lat_dim,string  filename_str)
	{
...
{
    return load_hdf5_externC( (char*) data, file_offset, size, sizeLocal, halo, lat_dim,  filename_str);
}
