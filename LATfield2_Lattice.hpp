...
Lattice::Lattice(int dim, const int* size, int halo, int vectorSize)
{
	status_=0;
	arch_saved_=false;
	this->initialize(dim, size, halo,vectorSize);
}
...
Lattice::Lattice(int dim, const int size, int halo, int vectorSize)
{
	...
	this->initialize(dim, sizeArray, halo,vectorSize);
	delete[] sizeArray;
}
...
void Lattice::initialize(int dim, const int size, int halo, int vectorSize)
{
	int* sizeArray=new int[dim];
	for(int i=0; i<dim; i++) { sizeArray[i]=size; }
	this->initialize(dim, sizeArray, halo, vectorSize);
...
long  Lattice::vectorSize() {return vectorSize_; }
